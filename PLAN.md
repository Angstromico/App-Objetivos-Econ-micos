...
`npx vue-tsc --noEmit` sin errores. Los tipos son importables desde cualquier otro módulo.
...
const progresoDeProyecto = computed(
  () =>
    (proyectoId: string): ProgresoProyecto => {
      ...
      const costoIdeal = itemsPendientes.reduce(
        (sum, i) => sum + i.presupuestoGama[i.gamaSeleccionada],
        0,
      )
